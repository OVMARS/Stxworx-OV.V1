   ...
   return (
      <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
         <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
               <h2 className="text-xl sm:text-2xl font-black text-white uppercase tracking-tight">Jobs Queue</h2>
               <p className="text-slate-400 text-xs sm:text-sm">Monitor active escrow contracts and disputes.</p>
            </div>
            <div className="flex flex-wrap gap-2 items-center w-full sm:w-auto">
               <input
                  ...
                  className="bg-[#0b0f19] border border-slate-800 rounded-lg px-3 py-2 text-sm text-white focus:border-orange-500 focus:outline-none flex-1 sm:flex-none sm:w-40"
               />
               <select
                  value={statusFilter}
                  onChange={(e) => setStatusFilter(e.target.value)}
                  className="bg-[#0b0f19] border border-slate-800 rounded-lg px-3 py-2 text-sm text-white focus:border-orange-500 focus:outline-none flex-1 sm:flex-none"
               >
               ...
               <button onClick={handleRefresh} className="p-2 bg-slate-800 hover:bg-slate-700 rounded-lg text-slate-400 hover:text-white transition-colors shrink-0">
            ...
            <table className="w-full text-left border-collapse min-w-[700px]">
               <thead>
                  <tr className="text-xs font-bold uppercase tracking-wider text-slate-500 border-b border-slate-800 bg-slate-900/50">
                     ...
                     <th className="px-3 sm:px-6 py-3 sm:py-4">Parties</th>
                     <th className="px-3 sm:px-6 py-3 sm:py-4">Budget</th>
                  ...
                  {projects.map((project) => {
                     ...
                     <React.Fragment key={project.id}>
                        <tr className={`hover:bg-slate-800/30 transition-colors ${expandedProjectId === numId ? 'bg-slate-800/20' : ''}`}>
                           <td className="px-3 sm:px-6 py-3 sm:py-4 text-xs font-mono text-slate-500">#{project.id}</td>
                           ...
                           <td className="px-3 sm:px-6 py-3 sm:py-4 text-xs text-slate-400">
                        ...
                        {expandedProjectId === numId && (
                           <tr>
                              <td colSpan={6} className="bg-slate-900/50 p-4 sm:p-6 border-b border-slate-800 shadow-inner">
